...
- **Alvo de build do piloto (2026-06-09): ADR-010** (`docs/decisions/ADR-010-piloto-portal-operadora.md`) + SDD do portal (`docs/specs/portal-operadora-piloto-sdd.md`) — **motor local inalterado + portal da operadora**. ADR-008 = visão futura **suspensa** (não revogada); ADR-009 = **dormente** (reativa se a pegada AWS voltar).
...
- **Portal da operadora (ADR-010):** FastAPI + Jinja2 + JS vanilla, monorepo em `src/socialselling/portal/` (módulo novo; a UI local `socialselling/web` **não muda** — vira utilitário de operador/dev). Hosting: **Render free + Neon Postgres free**. O motor **NUNCA acessa o banco** — só HTTP (`POST /api/publish` empurra snapshot; `GET /api/feedback` puxa eventos). O portal fica **fora do pipeline cognitivo** (M1–M5 intactos).
...
- ❌ Banco/fila/servidor **no motor** (ORM, migrations, Redis, Celery, RabbitMQ continuam proibidos). **Postgres é permitido EXCLUSIVAMENTE como storage do portal (ADR-010), acessado só pelo portal.** AWS/Terraform/Docker continuam fora do piloto.
- ❌ Score numérico exposto à operadora (mostrar drivers XAI, não números).
...
config/   runtime.toml, hypotheses_catalog.json, feedback_catalog.json, icp_criteria.example.json
...
src/socialselling/  skills/ modules/ core/ portal/ orchestrator.py   # portal/ = ADR-010
...
- **Portal local (dev, ADR-010 — após implementação):** `py -m uvicorn socialselling.portal.app:app --reload`. Deps: `pip install -e ".[portal]"`.
- **Publicar snapshot / puxar feedback (ADR-010 — após implementação):** `py -m socialselling.sync.publish --profile <id> [--dry-run]` · `py -m socialselling.sync.pull_feedback` (ver SDD do portal §6).
...
- **Plano vigente (piloto ADR-010):** `docs/planning/adr-010-backlog-plan.md`. **Política do processo agêntico:** ADR-011 (`docs/decisions/ADR-011-processo-agentico-de-referencia.md`) — modelos por tarefa, paralelização, anti-desperdício.
