...
3. Calcula P_score e atribui action_label:
   ...
   - Labels por threshold: `P_score >= 0.70` → `PRIORITY_ACTION`; `0.45 <= P < 0.70` → `MONITOR`; `0.25 <= P < 0.45` → `DELTA_SEARCH`; `P < 0.25` → `PRUNED`

4. Persiste `score_snapshots` com todos os componentes e atualiza `analytical_feature_store`
...
- Todos os componentes intermediários registrados em `score_snapshots` para auditoria
...
[Scheduled Monitor Job — executa a cada 30min]
         │
         │  Verifica leads com search_mode='DELTA'
         │  Consulta trigger_event_log por novos eventos
         │
...
[POST /api/v1/webhooks/crm] ── Chamado pelo CRM externo
         ...
         │
         ▼ [HMAC válido]
[crm_feedback_log INSERT — status='QUEUED']
...
[SRS Update Job — EV-18]
         ...
         │                                           keyword_weights atualizado
         │                                           no icp_contract (anti-overfitting)
         │
         ▼
