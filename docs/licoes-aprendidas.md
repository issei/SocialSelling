...
- **L-072 | Portal/Deploy | App FastAPI com factory (`create_portal_app(dao)`) NÃO expõe `app` no módulo — `uvicorn pkg.app:app` falha com "Attribute 'app' not found".** O start command do Render precisa de um módulo entrypoint (`portal/main.py`) que crie a instância: `app = create_portal_app(_build_dao())`. Bug pego em teste local (a spec apontava `portal.app:app`; corrigida no PR #110). Aplicar: todo app com factory tem um `main.py` com a instância nomeada; teste no gate que `import pkg.main; assert hasattr(mod,"app")`.
- **L-073 | Portal/Neon | Conexão Postgres única aberta no boot morre silenciosamente no Neon free (suspende conexões ociosas) → 500 no primeiro acesso após idle.** Solução: `ReconnectingPostgresDAO` com `_conn` como property que checa `closed` e reabre sob demanda — transparente para os métodos herdados. Aplicar: qualquer DAO de longa vida contra banco serverless free precisa de reconexão preguiçosa, não conexão persistente.
- **L-074 | Sync/Publish | O corpus ADR-006 persiste `LeadCard` (lossy): só `why_now` (drivers positivos) e `gaps` (missing_signals) sobrevivem — `negative_signals` e `references` de proveniência do `XAIPayload` se perdem.** A CLI publish mapeia o que existe e deixa `references=[]` (Open-World: não fabrica proveniência ausente). Se a carteira precisar de drivers ricos, uma WU futura deve persistir o `XAIPayload` no corpus. Decidido em WU-E3 sem bloquear (Gherkin não exigia drivers negativos).
